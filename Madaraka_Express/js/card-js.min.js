...
!function(tjq) {
    ...
    tjq.fn.CardJs = function(methodOrOptions) {
        return methods[methodOrOptions] ? methods[methodOrOptions].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof methodOrOptions && methodOrOptions ? void tjq.error("Method " + methodOrOptions + " does not exist on jQuery.CardJs") : methods.init.apply(this, arguments)
    }
}(jQuery),
tjq(function() {
    tjq(".card-js").each(function(i, obj) {
        tjq(obj).CardJs()
    })
}),
...
CardJs.copyAllElementAttributes = function(source, destination) {
    tjq.each(source[0].attributes, function(idx, attr) {
...
CardJs.handleMaskedNumberInputKey = function(e, mask) {
    ...
    if (isNumber || isDelete || isBackspace) {
        ...
        tjq(element).val(CardJs.applyFormatMask(numbersOnly, mask)),
...
CardJs.prototype.initCardNumberInput = function() {
    ...
    this.cardNumberInput[0] ? this.cardNumberInput.detach() : this.cardNumberInput = tjq("<input class='card-number' />"),
    this.cardNumberInput.attr("type", "tel"),
    this.cardNumberInput.attr("placeholder") || this.cardNumberInput.attr("placeholder", CardJs.CREDIT_CARD_NUMBER_PLACEHOLDER),
...
CardJs.prototype.initNameInput = function() {
    ...
    this.nameInput[0] ? (this.captureName = !0,
    this.nameInput.detach()) : this.nameInput = tjq("<input class='name' />"),
    this.nameInput.attr("placeholder") || this.nameInput.attr("placeholder", CardJs.NAME_PLACEHOLDER)
}
,
CardJs.prototype.initExpiryMonthInput = function() {
    this.expiryMonthInput = this.elem.find(".expiry-month"),
    this.expiryMonthInput[0] ? this.expiryMonthInput.detach() : this.expiryMonthInput = tjq("<input class='expiry-month' />")
}
,
CardJs.prototype.initExpiryYearInput = function() {
    this.expiryYearInput = this.elem.find(".expiry-year"),
    this.expiryYearInput[0] ? this.expiryYearInput.detach() : this.expiryYearInput = tjq("<input class='expiry-year' />")
}
,
CardJs.prototype.initCvcInput = function() {
    this.cvcInput = this.elem.find(".cvc"),
    this.cvcInput[0] ? this.cvcInput.detach() : this.cvcInput = tjq("<input class='cvc' />"),
    this.cvcInput.attr("type", "tel"),
    this.cvcInput.attr("placeholder") || this.cvcInput.attr("placeholder", CardJs.CVC_PLACEHOLDER),
    this.cvcInput.attr("maxlength", CardJs.CVC_MASK_3.length),
...
CardJs.prototype.setupExpiryInput = function() {
    ...
    if (this.EXPIRY_USE_DROPDOWNS) {
        ...
        for (var expiryYearNew = tjq("<select><option value='any' selected='' hidden=''>YY</option></select>"), currentYear = parseInt((new Date).getFullYear().toString().substring(2, 4)), i = 0; i < CardJs.EXPIRY_NUMBER_OF_YEARS; i++)
        ...
        expiryInput = tjq("<div></div>"),
        this.expiryMonthInput = tjq("<input type='hidden' name='expiry-month' />"),
        this.expiryYearInput = tjq("<input type='hidden' name='expiry-year' />"),
        this.stripe && (this.expiryMonthInput.attr("data-stripe", "exp-month"),
        this.expiryYearInput.attr("data-stripe", "exp-year")),
        this.expiryMonthYearInput = tjq("<input class='expiry' />"),
        this.expiryMonthYearInput.attr("type", "tel"),
        this.expiryMonthYearInput.attr("placeholder") || this.expiryMonthYearInput.attr("placeholder", CardJs.EXPIRY_PLACEHOLDER),
